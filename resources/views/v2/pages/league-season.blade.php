...
@section('content')
    <main class="tournaments-page league-season-page">
        <section class="tournaments-hero">
            <div class="container">
                <div>
                    <span class="tournaments-kicker">{{ __('League Centre') }}</span>
                    <h1>{{ $season['league'] }} {{ $season['year'] }}</h1>
                    <p>
                        {{ __('Standings, completed results and upcoming fixtures for this league season.') }}
                    </p>
                </div>
            </div>
        </section>
            ...
            <div class="container">
                <article class="tournament-card tournament-card-focused">
                    <header class="tournament-card-head">
                        <div>
                            <span>{{ $season['country'] ?: __('League season') }}</span>
                            <h2>{{ $season['league'] }}</h2>
                    ...
                    <div class="tournament-content-grid league-season-content-grid">
                        <section class="tournament-standings">
                            <div class="tournament-panel-title">
                                <h3>{{ __('League Table') }}</h3>
                                <span>{{ __('Sorted by points, goal difference and goals scored') }}</span>
                            </div>
                        ...
                        <aside class="tournaments-control-panel league-season-control-panel" aria-label="{{ __('League season navigation') }}">
                            <div class="standings-filter-field">
                                <span>{{ __('Season years') }}</span>
                            ...
                            <div class="standings-filter-field">
                                <span>{{ __('Leagues in :year', ['year' => $season['year']]) }}</span>
                                <nav class="token-roadmap-intro-actions" aria-label="{{ __('Available leagues') }}">
